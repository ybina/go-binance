...
func (s *websocketServiceTestSuite) TestWsAggTradeServe() {
	...
	doneC, stopC, err := WsAggTradeServe("ETHBTC", func(event *WsAggTradeEvent, policy *WsTradePolicy, triggerCh chan interface{}) {
		e := &WsAggTradeEvent{
			Event:                 "aggTrade",
			Time:                  1499405254326,
			Symbol:                "ETHBTC",
...
func (s *websocketServiceTestSuite) TestWsCombinedAggTradeServe() {
	...
	doneC, stopC, err := WsCombinedAggTradeServe([]string{"ETHBTC"}, func(event *WsAggTradeEvent, policy *WsTradePolicy, triggerCh chan interface{}) {
		e := &WsAggTradeEvent{
			Event:                 "aggTrade",
			Time:                  1499405254326,
			Symbol:                "ETHBTC",
